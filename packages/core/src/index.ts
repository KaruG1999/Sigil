...
export type Finding = { type: "info" | "warning" | "critical"; msg: string };

export async function scanRepository(target: string): Promise<{ score: number; findings: Finding[] }> {
  ...
  try {
    ...
    if (/^https?:\/\//.test(target)) {
      ...
      try {
        const rawUrl = (() => {
          ...
          if (m) return `https://raw.githubusercontent.com/${m[1]}/${m[2]}/main/package.json`;
          return null;
        })();
    ...
    for (const f of findings) {
      if (f.type === "warning") score -= 20;
      if (f.type === "critical") score -= 40;
    }
    score = Math.max(0, score);

    return { score, findings };
  } catch (err) {
    return { score: 0, findings: [{ type: "critical", msg: (err as Error).message || String(err) }] };
  }
}
